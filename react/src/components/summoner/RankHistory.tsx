...
function RankHistory(props: {summoner: SummonerType; theme: string}) {
  ...
  const [four_div_tiers] = useState(['IRON', 'BRONZE', 'SILVER', 'GOLD', 'PLATINUM', 'DIAMOND'])
        ...
        <div style={{display: 'inline-block'}}>
          ...
          >
            ...
            {[...four_div_tiers, ...one_div_tiers].map((tier, key) => {
              if (key > 0) {
                return <ReferenceLine key={key} y={4 * key} label={`${tier}`} stroke="#ffffff" />
              }
              return null
            })}
            ...
            <Tooltip
              formatter={(value: any, name: any, props: any) => {
                ...
                let output = `${trough.tier.toUpperCase()} ${trough.division} ${
                  trough.league_points
                } -to- ${peak.tier.toUpperCase()} ${peak.division} ${peak.league_points}`
                return [output, 'Rank']
              }}
