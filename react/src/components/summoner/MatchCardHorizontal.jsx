...
function MatchCard(props) {
        ...
        {team.map((part) => {
          ...
          return (
            <div key={`${match.id}-${part._id}`} style={{position: 'relative'}}>
              ...
              {/* DAMAGE PERCENTAGE */}
              <div
                title={`${numeral(part.stats.total_damage_dealt_to_champions).format(
                  '0,0',
                )} damage to champions`}
                style={{
                  position: 'absolute',
              ...
              >
                {is_me && (
                  <small style={{fontWeight: 'bold'}}>{formatName(part.summoner_name)}</small>
                )}
                {!is_me && (
                  <small>
                    {part.puuid !== '0' && (
                    ...
                    {part.puuid === '0' && (
                      <span title={part.summoner_name}>{formatName(part.summoner_name)}</span>
                    )}
                  </small>
                )}
              </div>
  ...
  if (mypart !== undefined) {
    return (
      <>
        <div style={{height: CARDHEIGHT}} className={`card-panel ${theme} match-card`}>
          <div className="win-loss-card-bar" style={{background: `${topBarColor()}`}}></div>
          <div
            style={{
              display: 'inline-block',
              paddingRight: 5,
          ...
          >
            <div>
              ...
              <div style={{display: 'inline-block', paddingLeft: 4}}>
                <img style={{height: 20, display: 'block'}} src={mypart.summoner_1_image} alt="" />
                <img style={{height: 20, display: 'block'}} src={mypart.summoner_2_image} alt="" />
              </div>
            </div>
            <img
          ...
          >
            ...
            <small style={{lineHeight: 1, display: 'inline-block', paddingLeft: 10}}>
              {`${Math.floor(game_time)}:${numeral((match.game_duration / 1000) % 60).format(
                '00',
              )}`}
            </small>
          </div>
          ...
          <div style={{display: 'block', position: 'absolute', bottom: 3, left: 26}}>
            <small className={`${store.state.theme} ${matchHighlightColor(match.queue_id)}`}>
              {props.queues[match.queue_id] && (
                <Fragment>{convertQueue(props.queues[match.queue_id].description)}</Fragment>
              )}
              {props.queues[match.queue_id] === undefined && <Fragment>{match.queue_id}</Fragment>}
            </small>
          </div>
          ...
          >
            <i className="material-icons" style={{position: 'absolute', bottom: 5, right: 8}}>
              arrow_downward
            </i>
          </Link>
        </div>
