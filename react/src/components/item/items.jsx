          ...
          <React.Fragment>
            ...
            {item.notes.length > 0 && (
              ...
              >
                <small>
                  {item.notes.map((note) => {
                    return (
                      <div style={{lineHeight: 1.3, display: 'inline-block'}} key={item._id}>
                        {note}
                      </div>
                    )
                  })}
            ...
            <div style={{marginBottom: 0}} className="row">
              {Object.keys(item.stats).map((key) => {
                ...
                let title = `${value} ${convertStatName(key)[0]} is worth approximately ${numeral(
                  stat.gold_value,
                ).format('0,0')} gold`
                return (
                  <div title={title} key={`${item.name}-${key}-${item.version}`} className="col s6">
            ...
            <div dangerouslySetInnerHTML={{__html: stripHtml(item.description)}}></div>
          </React.Fragment>
        )}
      </div>
    </LazyLoad>
