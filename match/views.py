...
class MatchDetailView(HtmxMixin, DetailView):  # type: ignore
    ...
    def get_context_data(self, **kwargs):
        ...
        context["serialized_participants"] = self.basic_participant_serializer(
            match.sorted_participants
        )
    ...
    def get_hr_data(self, match):
        ...
        user_puuids = set(
            Summoner.objects.filter(
                id__in=user.summonerlinks.all().values("summoner_id")
            ).values_list("puuid", flat=True)
        )
        if match_puuids & user_puuids:
            ...
            return Heartrate.objects.format_for_match(match, self.request.user)
        return []
